...
for (label in labels){
    ...
    for (set in unique(df$Set)) {
        ...
        if (length(unique(set_df$Flat)) > 1) {
            ...
            # Fit a linear model
            model <- lme4::lmer(paste0(label, ' ~ Genotype + (1|Column) + (1|Row) + (1|Flat)'), data = set_df)

            # Save the model
            saveRDS(model, paste0('../output/0_raw_data_correction/Set_', set,
        ...
        if (length(unique(set_df$Flat)) == 1) {
            ...
            tryCatch({
                # Fit a linear model
                model <- lme4::lmer(paste0(label, ' ~ Genotype + (1|Column) + (1|Row)'), data = set_df)
...
write.table(res, paste0('../data/double_mutant_fitness_data_05312024_all_corrected_brianna.txt'), row.names=F, quote=F, sep='\t')
